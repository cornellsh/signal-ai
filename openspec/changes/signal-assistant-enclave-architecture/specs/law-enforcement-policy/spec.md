...
The system SHALL PROVIDE limited host operational logs (timestamps, `internal_user_id`, message sizes, error codes) for up to 30 days, which do not contain conversation content or direct Signal IDs.
...
The system SHALL PROVIDE confirmation that communication occurred between the bot and a specific Signal ID at a certain time, without revealing content or purpose.
...
If a valid legal order specifically targets the mapping for a known `internal_user_id` or `Signal_ID`, and the enclave is running, the system SHALL RETRIEVE this mapping from within the running enclave via a specific enclave API call.
