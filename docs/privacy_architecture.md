...
| **Conversational Context/State** | Enclave (Assistant Bot Logic) | Enclave | Enclave Secure Storage (on Host, encrypted by ESSK) / Ephemeral in Enclave memory | Ephemeral (short term in Enclave memory); Long-term (user-defined or system-defined TTL, purged on user deletion). Persisted conversational context/state MUST consist only of redacted/sanitized summaries or embeddings equivalent to those permitted for Long-Term Memory. It MUST NOT contain additional raw or plaintext conversation attributes beyond those allowed for Long-Term Memory, and MUST follow the same TTL and deletion semantics. | TEE isolation, Encryption at rest (ESSK). | NO |
...
*   **Host (Untrusted):**
    ...
    *   **Data Visibility:** The Host MUST NEVER see plaintext user messages or LLM prompts/responses. The Host WILL see Signal network-level identifiers transiently on the wire (for routing and delivery), but MUST NOT persist raw `Signal_ID`s at rest. Any persisted metadata MUST use `internal_user_id` only and MUST NOT store `Signal_ID`s or plaintext content.
*   **Enclave (Trusted):**
...
*   **Attestation:** The enclave supports remote attestation, allowing users or auditors to cryptographically verify the exact code and configuration running inside a specific enclave. This provides verifiable assurance that the privacy guarantees outlined in this document are actively enforced. Any attempt to modify the enclave's behavior (e.g., to exfiltrate plaintext data) would change its cryptographic measurement, invalidating attestation and making the alteration detectable.
...
*   **Storage Encryption (Enclave-Managed):** All persistent data written by the enclave to the host's untrusted disk (e.g., identity mappings, Signal key material) is encrypted by the enclave using an enclave-generated Secure Storage Key. This key itself is sealed (encrypted) by the Enclave Root Key, ensuring that data at rest on the host is unintelligible without access to the enclave's secure environment. The only exception is strictly limited, pseudonymized host-side queues/caches documented in the Data Inventory table under “Signal Metadata (Non-Content)”. These queues/caches MUST contain only `internal_user_id` and non-identifying metadata, MUST NOT contain plaintext content or `Signal_ID`s, and are currently NOT encrypted at rest. This is a temporary implementation compromise; encryption at rest for this specific data type is marked as a potential future enhancement.
...
*   **Host Database/File System (Enclave-Encrypted):** This is the primary location for any persistent data. This includes:
    ...
    *   **Long-Term Memory (Redacted Summaries/Embeddings):** Stored encrypted using the Secure Storage Key, managed by the enclave, and linked to `internal_user_id`.
...
*   **Plaintext Conversation Content (User Messages, LLM Prompts/Responses):** **Never retained.** These are ephemeral and purged from enclave memory immediately after use. This ensures that, on the server side, past conversation content is practically unrecoverable even if server storage or long-term server-side keys are later compromised. End-to-end forward secrecy remains the responsibility of the Signal protocol itself.
...
*   Any conversational context/state that is persisted beyond ephemeral enclave memory MUST obey the same sanitization and retention constraints as Long-Term Memory.
...
*   **User-Initiated Deletion:** A user deletion request MUST result in: (a) deletion of the `(Signal_ID, internal_user_id)` mapping from enclave secure storage, (b) deletion of all Long-Term Memory and conversational context/state entries associated with that `internal_user_id`, and (c) deletion or scheduled deletion of any host-side metadata that is uniquely keyed by that `internal_user_id`, consistent with the Data Inventory and applicable retention/TTL policies.
...
*   **Requirement 9.1.2.2 (Proof of Communication):** The system CAN confirm that communication occurred between the bot and a specific `internal_user_id` at a certain time, but MUST NOT reveal the content or purpose of the communication. When combined with Host Operational Logs, such mappings allow reconstruction of communication timelines (e.g., when a given `internal_user_id` interacted with the assistant), but still do not reveal conversation content.
*   **Requirement 9.1.2.3 (Identity Mappings from Running Enclave):** If a valid legal order specifically targets the mapping for a known `internal_user_id` or `Signal_ID`, and the Enclave is currently running and accessible, the system CAN retrieve this mapping *from within the running Enclave* via a specific, authenticated Enclave API call. This mapping is stored encrypted by an ESSK (see Data Inventory row: **Signal User Identifier (UUID/Phone Number)**) and the Enclave will decrypt it for the sole purpose of responding to a valid legal order. This operation MUST only be available via a dedicated, auditable control path requiring multi-party authorization (e.g., an offline process or separate control service with at least two independent approvals, such as legal and security). It MUST NOT be exposed as a generic programmatic API to the Host or to routine operational code paths. If the Enclave is not running, or if its secure storage has been purged (due to an ERK compromise, for example, as per [Signal Assistant Core Specification, Section 9.3.2: Compromise Handling and Blast Radius](signal_assistant_core.md#93-rotation-and-compromise-handling)), this mapping MUST NOT be recoverable by a subsequent *attested* Enclave instance.
...
All enclave code paths capable of accessing identity mappings, extended views of Host Operational Logs, or other LE-sensitive internal state MUST first invoke `CHECK_LE_POLICY(request_type, target, authorization_context)` and MUST honor its ALLOW/DENY decision. `CHECK_LE_POLICY` MUST validate the legal basis and authorization context for the request and MUST default to DENY in ambiguous cases.
...
*   **11.2.1 Enclave Isolation (Mitigates Malicious Host Operator, Insider Threat, Side-Channel):**
    ...
    *   **Mechanism:** TEE hardware protections and enclave memory isolation significantly raise the bar for such attacks but do not eliminate all possible side channels. Side-channel hardening beyond the TEE’s default guarantees is considered out of scope for the current design.
...
*   **11.2.7 Remote Attestation (Mitigates Malicious Host Operator, Abusive LE Request):**
    ...
    *   **Mechanism:** Hardware-backed attestation primitives, cryptographic measurement of Enclave state. There MUST NOT exist any alternative (“debug”, “emergency override”, or similar) key-release path that bypasses attestation measurement checks or allows cryptographic keys (SPK, ESSK, ERK, or external API keys) to be used by un-attested or unauthorized code.
...
*   **Data Deletion Requests:** Users will have a clear mechanism to request the deletion of their associated data. Upon such a request, the enclave will irrevocably purge their `Signal_ID` to `internal_user_id` mapping and any associated encrypted state, effectively making the user's past interactions unrecoverable.
